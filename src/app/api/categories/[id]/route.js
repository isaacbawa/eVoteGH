...
const ALLOWED_UPDATES = ['name', 'slug', 'description', 'cover_image_url', 'display_order', 'is_active'];

export async function GET(_request, { params }) {
  try {
    const { id } = await params;
    const result = await query(`SELECT * ${DATE_ALIASES} FROM categories WHERE id = $1`, [id]);
