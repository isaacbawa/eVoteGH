...
export async function GET(_request, { params }) {
  ...
  try {
    ...
    const result = await query(`SELECT * ${DATE_ALIASES} FROM organizers WHERE id = $1`, [id]);
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const result = await query(
      `UPDATE organizers SET ${updates.join(', ')} WHERE id = $${idx}
...
export async function DELETE(_request, { params }) {
  ...
  try {
    const { id } = await params;
    await query('DELETE FROM organizers WHERE id = $1', [id]);
