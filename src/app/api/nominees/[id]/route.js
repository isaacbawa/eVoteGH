...
const ALLOWED_UPDATES = [
  'category_id', 'user_id', 'name', 'slug', 'bio', 'photo_url',
...
export async function GET(_request, { params }) {
  try {
    ...
    const result = await query(`SELECT * ${DATE_ALIASES} FROM nominees WHERE id = $1`, [id]);
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const result = await query(
      `UPDATE nominees SET ${updates.join(', ')} WHERE id = $${idx}
